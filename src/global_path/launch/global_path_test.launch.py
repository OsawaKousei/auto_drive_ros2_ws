...
def generate_launch_description() -> LaunchDescription:
    ...
    )  # setting for lifecycle_manager

    # get package directory
    pkg_share_dir = get_package_share_directory('global_path')

    # get rviz config file path
    rviz_config_dir = os.path.join(pkg_share_dir, 'config', 'path_visualization.rviz')
    ...
    # get map file path
    map_file_path = os.path.join(pkg_share_dir, 'maps', 'map.yaml')
    ...
    return LaunchDescription(
        [rviz2, map_server, map_lifecycle_manager, path_visualize_node]
    )
