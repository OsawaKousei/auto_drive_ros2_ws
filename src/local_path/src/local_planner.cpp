...
namespace local_path
{
      ...
      : rclcpp::Node("local_planner", options)
  {
    ...
    path_sub_ = create_subscription<nav_msgs::msg::Path>("global_path", 1, [this](const nav_msgs::msg::Path::SharedPtr msg)
                                                         { path_callback(msg); });
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("odom", 1, [this](const nav_msgs::msg::Odometry::SharedPtr msg)
                                                             { this->odom_ = *msg; });
