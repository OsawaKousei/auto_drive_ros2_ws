...
from math import atan2, sin, tan
...
class PurePursuitController:
    ...
    def _calculate_look_ahead_distance(self, state):
        ...
        self.look_ahead_distance_m = (
            self.LOOK_FORWARD_GAIN * state.get_speed_mps()
    ...
    def _calculate_target_course_index(self, state):
        ...
        while self.look_ahead_distance_m > self.course.calculate_distance_from_point(
            state, nearest_index
        ):
    ...
    def _calculate_target_steer_angle_rad(self, state):
        ...
        diff_angle_rad = self.course.calculate_angle_difference_rad(
            state, self.target_course_index
        )
        self.target_steer_rad = atan2(
            (2 * self.WHEEL_BASE_M * sin(diff_angle_rad)), self.look_ahead_distance_m
        )
    ...
    def _calculate_target_yaw_rate_rps(self, state):
        ...
        self.target_yaw_rate_rps = (
            state.get_speed_mps() * tan(self.target_steer_rad) / self.WHEEL_BASE_M
        )
